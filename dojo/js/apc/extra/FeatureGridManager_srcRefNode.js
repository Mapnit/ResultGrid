define([
	...
	dom, domStyle, ContentPane, registry, 
	QueryTask, Query, 
	...
	fgm.options = {
		...
		title: "Query Results", 
		map: null, 
		srcRefNode: null, // wrapper html element given by the caller
		highlightSymbols: {
			"point": {
				"type": "esriSMS",
				"style": "esriSMSCircle",
	...
	fgm.buildFeatureGrid = function(searchParams, options) {
		...
		var isSrcRefNodeValid = dom.byId(fgm.options.srcRefNode) && registry.byId(fgm.options.srcRefNode); 
		if (isSrcRefNodeValid) {
	...
	fgm._buildResultContentPane = function(/*container pane*/) {
		var containerDom = dom.byId(fgm.options.srcRefNode);
		var containerWidget = registry.byId(fgm.options.srcRefNode); 
		var cs = domStyle.getComputedStyle(containerDom);
		var w = cs.width, h = cs.height;

		var contentHtml = '<div id="fgm-resultSplitter" style="height:98%; overflow:hidden">'
			+ '<div id="fgm-layerPanelbar"></div>' 
			+ '<div id="fgm-datagrid"></div>'
			+ '</div>'; 
		fgm._resultWindow = new ContentPane({
			content: contentHtml,
			region:'bottom', 
		...
		fgm._resultWindow.startup(); 

		containerWidget.addChild(fgm._resultWindow); 

		//buildResultPanel(); 

		aspect.after(fgm._resultWindow, "resize", function(newSize){
			if (newSize) {
				fgm._resizePanesInContentPane(newSize); 
			} 
		}, true);  

		//resizeResultPane({h:parseFloat(h)/3, w:parseFloat(w)}); 
		fgm._resizePanesInContentPane({w:parseFloat(w)}); 
	}; 
	...
	fgm._buildResultWindow = function() {
		...
		var onOpen = function(e) {
			var wndParent = $("#map");
			this.wrapper.css({ top: wndParent.height() - 500 - 50, left: 5 });
		}; 

		if (!resultWin.data("kendoWindow")) {
			var wndParent = $("#map"); 
            var wndWidth = wndParent.width() - 10 + "px",
				wndHeight = "500" + "px";

			resultWin.kendoWindow({
				width: wndWidth /*"1050px"*/,
				height: wndHeight /*"500px"*/,
				title: fgm.options.title,
				actions: [
					//"Pin",
					"Minimize",
	...
	fgm._removeFeatureGrid = function() {
		...
		var isSrcRefNodeValid = dom.byId(fgm.options.srcRefNode) && registry.byId(fgm.options.srcRefNode); 
		if (isSrcRefNodeValid) {
			var container = registry.byId(fgm.options.srcRefNode); 
			if (fgm._resultWindow) {
				container.removeChild(fgm._resultWindow); 
			}
		} else {
	...
	fgm._resizePanesInContentPane = function(newSize) {
		var kdoElement = $("#fgm-resultSplitter");
		if (newSize.h) {
